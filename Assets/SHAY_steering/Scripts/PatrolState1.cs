...
public class PatrolState1 : State1 {
	...
	public float changeStateTime, changeStateTimer = 10f; //A very simple change state condition!
	GameObject buzzer;
	
	public PatrolState1(GameObject myGameObject):base (myGameObject) //constructor that needs the same argument as the State base class constructor. use :base(GameObject) to inherit the same myGameObject reference, so this class can access the gameobject it's refereing to
	{
	...
	public override void Enter() //override runs over the base class abstract method of the same name (abstract methods can't handle functionality, they are only a blueprint)
	{
		...
		myGameObject.GetComponent<SteeringBehaviours1>().pathFollowEnabled = true;
		...
		Debug.Log("FlyBoids are patrolling!");
	}
